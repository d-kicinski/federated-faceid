...
from fedfaceid.settings import DataSettings, ModelSettings, FederatedSettings
...
def parse_args():
    ...
    parser.add_argument("--dataset_local_dir", type=lambda p: Path(p),
                        default=DataSettings.dataset_local_dir)
    parser.add_argument("--dataset_local_csv_file", type=lambda p: Path(p),
                        default=DataSettings.dataset_local_csv_file)

    parser.add_argument("--dataset_remote_dir", type=lambda p: Path(p),
                        default=DataSettings.dataset_remote_dir)
    parser.add_argument("--dataset_remote_csv_file", type=lambda p: Path(p),
    ...
    parser.add_argument("--people_per_batch", default=ModelSettings.people_per_batch, type=int)
    parser.add_argument("--images_per_person", default=ModelSettings.images_per_person, type=int)
    parser.add_argument("--batches_in_epoch", default=ModelSettings.batches_in_epoch, type=int)

    parser.add_argument("--num_local_images_to_use", type=int,
                        default=ModelSettings.num_local_images_to_use)
    parser.add_argument("--num_remote_images_to_use", type=int,
                        default=ModelSettings.num_remote_images_to_use)

    # Federated settings
    parser.add_argument('--num_global_epochs', type=int,
                        default=FederatedSettings.num_global_epochs)
    parser.add_argument('--num_global_batch', type=int,
                        default=FederatedSettings.num_global_batch)
    parser.add_argument('--num_users', type=int,
                        default=FederatedSettings.num_users,
                        help="number of users: K")
    parser.add_argument('--user_fraction', type=float,
...
def main():
    ...
    settings_model = ModelSettings(lfw_batch_size=args.lfw_batch_size,
    ...
    settings_federated = FederatedSettings(num_global_batch=args.num_global_batch,
                                           num_global_epochs=args.num_global_epochs,
                                           num_local_epochs=args.num_local_epochs,
                                           num_users=args.num_users,
                                           user_fraction=args.user_fraction)

    distributed_training.train(settings_data=settings_data,
