...
def evaluate_model(settings_model: ModelSettings, settings_data: DataSettings):
    ...
    checkpoint = load_checkpoint(checkpoint_path=settings_data.checkpoint_path, model=model)
    ...
    metrics: EvaluationMetrics = evaluate(model, distance_l2, data_loader_validate, figure_path)
...
def parse_args():
    ...
    parser.add_argument("--lfw_batch_size", type=int,
                        default=ModelSettings.lfw_batch_size)
...
def main():
    args = parse_args()
    model_settings = ModelSettings(model_architecture=args.model_architecture,
