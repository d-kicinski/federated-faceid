...
def generate_triplets(data_meta_path: str, num_triplets: int, output_path: str):
    metadata = pd.read_csv(
        data_meta_path, dtype={"id": object, "name": object, "class": int}
    )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Prepare triplets for FaceNet triplet " "loss training"
    )
    parser.add_argument("--data_meta_path", type=str, required=True)
    parser.add_argument("--output_path", type=str, required=True)
    parser.add_argument(
        "--num_triplets", type=int, required=False, default=NUM_TRIPLETS
    )
